...
import {buildModel,Model} from './maze/model';
import {buildView} from './maze/view';
import {buildMaze} from './maze/lib/main';
import {buildStateMachine, STATE_INIT, STATE_DISPLAYING, STATE_PLAYING, STATE_MASKING, STATE_DISTANCE_MAPPING, STATE_RUNNING_ALGORITHM} from './maze/stateMachine';
...
export class AppComponent implements OnInit, AfterViewInit{
  ...
  private init() {
    ...
    this.view.on(EVENT_SHOW_MAP_BUTTON_CLICKED, () => {
      this.stateMachine.distanceMapping();
        const [startCell, _1] = this.findStartAndEndCells(),
            coords = (startCell || this.model.maze.randomCell()).coords;
    ...
    this. view.on(EVENT_KEY_PRESS, this.ifStateIs(STATE_PLAYING).then((event: any) => {
    ...
    this.view.on(EVENT_DOWNLOAD_CLICKED, () => {
        function saveSvg(svgEl: any, name: any) {
        ...
        const svgDrawingSurface = drawingSurfaces.svg({el: elSvg}),
            fileName = `maze_${this.model.shape}_${Object.values(this.model.size).join('_')}_${this.model.randomSeed}.svg`;
...
/*
    ...
    maze.on(EVENT_CLICK, (...states: any) =>{
      return {
        then(handler: any) {
          return (event: any) => {
  ...
  private onAlgorithmChanged(algorithmId: any) {
  ...
  private setupAlgorithms() {
    ...
    this.view.on(EVENT_ALGORITHM_SELECTED, this.onAlgorithmChanged);
  }
  ...
  private setupExitConfigs() {
    ...
    this.view.on(EVENT_EXITS_SELECTED, (exitConfig: any) => {
  ...
  private ifStateIs(stateMachine: any, ...states: any) {
    return {
      then(handler: any) {
        return (event: any) => {
          if (states.includes(stateMachine.state)) {
