...
import {buildStateMachine, STATE_INIT, STATE_DISPLAYING, STATE_PLAYING, STATE_MASKING, STATE_DISTANCE_MAPPING, STATE_RUNNING_ALGORITHM} from './stateMachine';
...
import {config, Shape} from './config';
import {algorithms} from './lib/algorithms';
import {buildRandom} from './lib/random';
import {
...
window.onload = () => {
  ...
  function isMaskAvailableForCurrentConfig() {
    const modelMaskKey = getModelMaskKey();
    let currentMask;
    if(modelMaskKey) {
      currentMask = model.mask[modelMaskKey];
    } 
  ...
  function setupShapeParameter() {
      ...
      function onShapeSelected(shapeName: Shape) {
      ...
      view.on(EVENT_MAZE_SHAPE_SELECTED, (shapeName: Shape) => {
  ...
  function setupSizeParameters() {
      ...
      function onParameterChanged(name: string, value: number) {
      ...
      view.on(EVENT_SIZE_PARAMETER_CHANGED, (data: any) => {
  ...
  function setupAlgorithms() {
      ...
      function onAlgorithmChanged(algorithmId: string) {
  ...
  function setupAlgorithmDelay() {
    ...
    view.on(EVENT_DELAY_SELECTED, (algorithmDelay: number) => {
  ...
  function setupExitConfigs() {
    ...
    view.on(EVENT_EXITS_SELECTED, (exitConfig: string) => {
  ...
  function buildMazeUsingModel(overrides: any={}) {
    if (model.maze) {
      model.maze.dispose();
    }
    ...
    maze.on(EVENT_CLICK, ifStateIs(STATE_DISTANCE_MAPPING).then((event: any) => {
      maze.findDistancesFrom(...event.coords);
      maze.render();
    }));

    maze.on(EVENT_CLICK, ifStateIs(STATE_MASKING).then((event: any) => {
      const cell = maze.getCellByCoordinates(event.coords);
      cell.metadata[METADATA_MASKED] = !cell.metadata[METADATA_MASKED];
      maze.render();
    }));

    maze.on(EVENT_CLICK, ifStateIs(STATE_PLAYING).then((event: any) => {
  ...
  function showEmptyGrid(deleteMaskedCells: any) {
  ...
  stateMachine.onStateChange((newState: any) => {
  ...
  view.on(EVENT_CREATE_MASK_BUTTON_CLICKED, () => {
      ...
      showEmptyGrid(false);
      (model.mask[getModelMaskKey()] || []).forEach((maskedCoords: any) => {
  ...
  function validateMask() {
      const isNotMasked = (cell: any) => !cell.metadata[METADATA_MASKED],
          startCell = model.maze.randomCell(isNotMasked);
      let unmaskedCellCount = 0;

      model.maze.forEachCell((cell: any) => {
          if (isNotMasked(cell)) {
              unmaskedCellCount++;
          }
      });
      ...
      function countUnmasked(cell: any) {
          cell.metadata[METADATA_VISITED] = true;
          let count = 1;
          cell.neighbours.toArray(isNotMasked).forEach((neighbourCell: any) => {
  ...
  view.on(EVENT_SAVE_MASK_BUTTON_CLICKED, () => {
      try {
          validateMask();
          stateMachine.init();
          const mask: any[] = model.mask[getModelMaskKey()] = [];
          model.maze.forEachCell((cell: any) => {
  ...
  function navigate(direction: any, shift: any, alt: any) {
    while (true) {
        const currentCell = model.playState.currentCell,
        ...
        if (moveOk) {
            ...
            if (model.playState.finished) {
                ...
                if (linkedDirections.length === 2) {
                    direction = linkedDirections.find((neighbourDirection: any) => targetCell.neighbours[neighbourDirection] !== model.playState.previousCell);
                } else {
                    break;
                }
            }
  ...
  view.on(EVENT_KEY_PRESS, ifStateIs(STATE_PLAYING).then((event: any) => {
  ...
  view.on(EVENT_DOWNLOAD_CLICKED, () => {
      function saveSvg(svgEl: SVGGraphicsElement, name: string) {
