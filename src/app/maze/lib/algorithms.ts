...
function markAsVisited(cell: any) {
  cell.metadata[METADATA_VISITED] = true;
}

function isVisited(cell: any) {
  return cell.metadata[METADATA_VISITED];
}

function isUnvisited(cell: any) {
  return !isVisited(cell);
}

function algorithmProgress(grid: any) {
  let previousCells: any[];

  grid.forEachCell((cell: any) => cell.metadata[METADATA_UNPROCESSED_CELL] = true);

  return {
    step(...cells: any) {
      this.current(...cells);
      cells.forEach((cell: any) => delete cell.metadata[METADATA_UNPROCESSED_CELL]);
    },
    current(...cells: any) {
      (previousCells || []).forEach((previousCell: any) => delete previousCell.metadata[METADATA_CURRENT_CELL]);
      cells.forEach((cell: any) => cell.metadata[METADATA_CURRENT_CELL] = true);
      previousCells = cells;
    },
    finished() {
...
export const algorithms = {
  ...
  [ALGORITHM_ALDOUS_BRODER]: {
      ...
      fn: function*(grid: any, config: any) {
          ...
          let unvisitedCount = grid.cellCount, currentCell: any;
  ...
  [ALGORITHM_WILSON]: {
      ...
      fn: function*(grid: any, config: any) {
          ...
          function markVisited(cell: any) {
              progress.step(cell);
              cell.metadata[METADATA_VISITED] = true;
          }
          function removeLoops(cells: any) {
              const latestCell = cells[cells.length - 1],
                  indexOfPreviousVisit = cells.findIndex((cell: any) => cell === latestCell);
          ...
          while (currentCell = grid.randomCell(isUnvisited)) {
              ...
              while (true) {
                  const nextCell: any = currentCell.neighbours.random();
      ...
      fn: function*(grid: any, config: any) {
          ...
          while (true) {
              ...
              if (nextCell) {
                  ...
                  const unvisitedCellWithVisitedNeighbours = grid.randomCell((cell: any) => isUnvisited(cell) && cell.neighbours.random(isVisited));
  ...
  [ALGORITHM_RECURSIVE_BACKTRACK]: {
      ...
      fn: function*(grid: any) {
          "use strict";
          const stack: any[] = [], progress = algorithmProgress(grid);
          let currentCell: any;
  ...
  [ALGORITHM_KRUSKAL]: {
      ...
      fn: function*(grid: any, config: any) {
          ...
          function mergeSets(id1: any, id2: any) {
              connectedSets[id2].forEach((cell: any) => {
  ...
  [ALGORITHM_TRUE_PRIMS]: {
      ...
      fn: function*(grid: any, config: any) {
          ...
          function getCellWithLowestCost(cells: any) {
  ...
  [ALGORITHM_ELLERS]: {
      ...
      fn: function*(grid: any, config: any) {
          ...
          function addCellToSet(setId: any, cell: any) {
          ...
          function mergeSets(setId1: any, setId2: any) {
              ...
              set2.forEach((cell: any) => addCellToSet(setId1, cell));
              delete sets[setId2];
          }

          function linkToCellBelow(cell: any) {
          ...
          for (let y = 0; y < height; y++) {
              ...
              if (isLastRow) {
                  ...
                  Object.keys(cellsInRowBySet).forEach(setId => {
                      random.shuffle(cellsInRowBySet[setId]).forEach((cell: any, i: number) => {
                          if (i === 0) {
