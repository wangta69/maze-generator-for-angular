...
export type Cell= { //TODO move methods outside so we only have 1 copy of each function
  ...
  neighbours: {
      ...
      linkedDirections() {
          return this.toArray().filter((neighbour: any) => neighbour.isLinkedTo(cell)).map((linkedNeighbour: any) => Object.keys(this).find(direction => this[direction] === linkedNeighbour));
      }
  },
  isLinkedTo(otherCell: any) {
    if(otherCell) {
...
/*
function buildBaseGrid(config: any) {
  "use strict";
  const cells: any = {}, {random} = config;

  function makeIdFromCoords(coords: any) {
      return coords.join(',');
  }
  ...
  function removeNeighbour(cell: any, neighbour: any) {
  ...
  function removeNeighbours(cell: any) {
      cell.neighbours.toArray().forEach((neighbour: any) => {
  ...
  return {
      ...
      getAllCellCoords() {
          const allCoords: any[] = [];
          this.forEachCell((cell: any) => allCoords.push(cell.coords));
          return allCoords;
      },
      link(cell1: any, cell2: any) {
      ...
      getCellByCoordinates(...coords: any) {
          const id = makeIdFromCoords(coords);
          return cells[id];
      },
      get cellCount() {
      ...
      findPathBetween(fromCoords: any, toCoords: any) {
          ...
          while(currentCell !== endCell) {
            const currentDistance = currentCell.metadata[METADATA_DISTANCE],
                nextCell: any = Object.values(currentCell.neighbours)
      ...
      findDistancesFrom(...coords: any) {
          ...
          while(frontier.length) {
              ...
              linkedUndistancedNeighbours.forEach((neighbour: any) => {
      ...
      clearMetadata(...keys: any) {
          keys.forEach((key: any) => {
              ...
              this.forEachCell((cell: any) => delete cell.metadata[key]);
          });
      },
      dispose() {
          eventTarget.off();
...
function buildBaseGrid(config: any): Grid {
  "use strict";
  const cells: any = {}, {random} = config;
...
export function buildSquareGrid(config: any) {
  "use strict";
  ...
  grid.render = function(drawingSurface = defaultDrawingSurface) {
      function drawFilledSquare(p1x: number, p1y: number, p2x: number, p2y: number, p3x: number, p3y: number, p4x: number, p4y: number, cell: any) {
  ...
  function findHardestExits() {
      ...
      function findFurthestEdgeCellFrom(startCell: any) {
          let maxDistance = 0, furthestEdgeCell: any[] = [];

          grid.findDistancesFrom(startCell.coords);

          edgeCells.forEach((edgeCell: any) => {
      ...
      const tmpStartCell = config.random.choice(edgeCells),
          endCell: any = findFurthestEdgeCellFrom(tmpStartCell),
          startCell: any = findFurthestEdgeCellFrom(endCell);
  ...
  grid.getClosestDirectionForClick = function(cell: any, clickEvent: any) {
...
export function buildTriangularGrid(config: any) {
  "use strict";
  const {drawingSurface: defaultDrawingSurface} = config,
      grid: any = buildBaseGrid(config),
  ...
  grid.render = function(drawingSurface = defaultDrawingSurface) {
      ...
      function drawFilledTriangle(p1x: number, p1y: number, p2x: number, p2y: number, p3x: number, p3y: number, cell: any) {
          drawingSurface.setColour(getCellBackgroundColour(cell, grid));
      ...
      if (path) {
          ...
          for (let i = 0; i < path.length; i++) {
              const
                  ...
                  [p1x, p1y, p2x, p2y, p3x, p3y] = getCornerCoords(...(currentCellCoords as [number, number]));
  ...
  function findHardestExits() {
      ...
      function findRandomMissingNeighbourDirection(cell: any) {
...
export function buildHexagonalGrid(config: any) {
  "use strict";
  const {drawingSurface: defaultDrawingSurface} = config,
      grid: any = buildBaseGrid(config);
  ...
  grid.render = function(drawingSurface = defaultDrawingSurface) {
      ...
      function drawFilledHexagon(p1x: number, p1y: number, p2x: number, p2y: number, p3x: number, p3y: number, p4x: number, p4y: number, p5x: number, p5y: number, p6x: number, p6y: number, cell: any) {
          drawingSurface.setColour(getCellBackgroundColour(cell, grid));
  ...
  function findHardestExits() {
      ...
      const tmpStartCell = config.random.choice(edgeCells),
          endCell: Cell = findFurthestEdgeCellFrom(tmpStartCell),
...
export function buildCircularGrid(config: any) {
  "use strict";
  ...
  function cellCountsForLayers(layers: any) {
  ...
  function findHardestExits() {
      ...
      const endCell = grid.getCellByCoordinates(0, 0),
          startCell: Cell = findFurthestEdgeCellFrom(endCell);
  ...
  grid.getClosestDirectionForClick = function(cell: Cell, clickEvent: any) {
      const cellCoords = cell.metadata[METADATA_RAW_COORDS],
          ...
          [startAngle, endAngle, _1, _2] = getCellCoords(...(cell.coords as [any, any])),
